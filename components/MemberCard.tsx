...
export function getMemberStatusRingClass(user: {
    ...
    const isDriving = Boolean(user.isDriving ?? (user.status === 'Driving' || (user.speed || 0) >= 15 || (user.status === 'Moving' && (user.speed || 0) > 5)));
    const isStationary = Boolean(user.isStationary ?? (user.status === 'Stationary' || (user.speed || 0) <= 0.6));
    const atSavedPlace = Boolean(user.atSavedPlace ?? Boolean(user.currentPlace));
    ...
    // 4. Default: Preserve layout with matching 2px border and 2px padding
...
export const MemberAvatarWithRing: React.FC<{
    ...
    const ringClass = getMemberStatusRingClass(member);
        ...
        <div className={`relative shrink-0 ${className}`}>
            ...
            {(renderBatteryBadge || renderStatusBadge) && (
                <div
                    ...
                    title={`${member.currentPlace ? `${member.currentPlace} • ` : ''}${member.status || 'Location'} • Battery: ${batteryVal}%${isCharging ? ' (Charging)' : ''}`}
                >
                    ...
                    {renderStatusBadge && (
                        <div
                            ...
                            title={member.currentPlace || member.status || 'Location'}
                        >
                            {isUnresolved ? (
                            ...
                            ) : member.currentPlace && member.status === 'Stationary' ? (
                                (member.currentPlace.toLowerCase().includes('work') || member.currentPlace.toLowerCase().includes('office')) ? (
...
export const MemberCard: React.FC<MemberCardProps> = ({
    ...
    const isUnresolved = member.locationStale === true;
