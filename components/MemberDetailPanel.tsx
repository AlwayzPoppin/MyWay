    ...
    <div className={`w-72 backdrop-blur-xl rounded-[2.5rem] shadow-2xl border overflow-hidden animate-in slide-in-from-left duration-300 ${panelBg}`}>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <img 
              ...
              alt={member.name} 
              className={`w-12 h-12 rounded-2xl border-2 border-white/30 object-cover bg-slate-800 ${member.isGhostMode ? 'grayscale opacity-50' : ''}`} 
            />
      ...
      <div className="p-5 space-y-5">
        ...
        {onToggleGhost && (
          ...
          >
            <span>{member.isGhostMode ? '👁️ Stop Invisible' : '👻 Go Invisible'}</span>
          </button>
        )}
