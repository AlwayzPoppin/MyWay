...
import { Bot, Clock, Flag, Route, X, Navigation } from 'lucide-react';
...
const NavigationOverlay: React.FC<NavigationOverlayProps> = ({ route, onCancel, theme }) => {
  ...
  return (
    <div className={`relative z-30 isolate backdrop-blur-3xl rounded-t-[2.5rem] md:rounded-[3rem] p-6 md:p-8 shadow-[0_-20px_80px_rgba(0,0,0,0.5)] border animate-in slide-in-from-bottom duration-700 ease-out safe-bottom ${bgColor}`}>
      ...
      <div className="flex justify-between items-start mb-6 md:mb-8">
        <div className="flex items-center gap-4 md:gap-8">
          <div className={`w-16 h-16 md:w-24 md:h-24 rounded-2xl md:rounded-[2.5rem] flex items-center justify-center shadow-2xl bg-indigo-600 ring-4 md:ring-8 ring-indigo-500/10 shrink-0`}>
            <Navigation className="w-8 h-8 md:w-12 md:h-12 text-white" />
          </div>
          <div>
        ...
        <button 
          onClick={onCancel} 
          className={`p-4 md:p-5 rounded-full transition-all border shadow-lg cursor-pointer ${theme === 'dark' ? 'bg-white/5 border-white/10 text-red-400 hover:bg-white/10' : 'bg-slate-100 border-slate-200 text-red-600 hover:bg-slate-200'}`}
      ...
      <div className="grid grid-cols-3 gap-2 md:gap-8 py-4 md:py-8 border-y border-white/5 bg-white/5 rounded-2xl md:rounded-3xl mb-4 md:mb-6">
        <div className="text-center flex flex-col items-center">
          <div className="flex items-center gap-1 mb-1 md:mb-2 text-slate-500">
            <Clock className="w-3 h-3 md:w-3.5 md:h-3.5 text-emerald-400" />
            <p className="text-[9px] md:text-[12px] uppercase font-black tracking-[0.1em]">Remaining</p>
          </div>
          <p className="text-lg md:text-3xl font-black text-green-400">{route.totalTime}</p>
        </div>
        <div className="text-center border-x border-white/10 px-1 flex flex-col items-center">
          <div className="flex items-center gap-1 mb-1 md:mb-2 text-slate-500">
            <Flag className="w-3 h-3 md:w-3.5 md:h-3.5 text-indigo-400" />
            <p className="text-[9px] md:text-[12px] uppercase font-black tracking-[0.1em]">Arrival</p>
          </div>
          <p className={`text-lg md:text-3xl font-black ${textColor}`}>4:52 PM</p>
        </div>
        <div className="text-center flex flex-col items-center">
          <div className="flex items-center gap-1 mb-1 md:mb-2 text-slate-500">
            <Route className="w-3 h-3 md:w-3.5 md:h-3.5 text-sky-400" />
            <p className="text-[9px] md:text-[12px] uppercase font-black tracking-[0.1em]">Distance</p>
          </div>
      ...
      {route.safetyAdvisory && (
        <div className={`p-4 md:p-6 rounded-2xl md:rounded-3xl flex items-center gap-4 md:gap-6 border ${theme === 'dark' ? 'bg-indigo-500/5 border-indigo-500/20' : 'bg-indigo-50 border-indigo-100'}`}>
          <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-indigo-600 flex items-center justify-center shadow-lg shrink-0 text-white">
