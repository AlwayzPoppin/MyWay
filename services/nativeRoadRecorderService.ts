...
export type RoadRecorderDiagnostic = { status: 'ready' | 'recording' | 'permission_required' | 'camera_busy' | 'camera_unavailable' | 'unsupported'; message?: string };
...
interface NativeRoadRecorderPlugin {
  ...
  getDiagnostics(): Promise<RoadRecorderDiagnostic>;
...
export const nativeRoadRecorderService = {
  ...
  getDiagnostics: () => NativeRoadRecorder.getDiagnostics(),
  requestCameraPermission: () => NativeRoadRecorder.requestCameraPermission(),
